...
// Client ID extraído de la solicitud del usuario
...
interface Lead {
  ...
  type: LeadType;
  status: LeadStatus;
  currentStep: number; // 0 = no iniciado, 1-4 = pasos de la campaña
...
function HakaTracker() {
  const [leads, setLeads] = useState<Lead[]>(() => {
    try {
      const saved = localStorage.getItem('hakalab_leads_v4');
  ...
  useEffect(() => {
    localStorage.setItem('hakalab_leads_v4', JSON.stringify(leads));
  }, [leads]);
  ...
  // Agrupar leads para el Dashboard
  const dashboardData = useMemo(() => {
    ...
    return {
      ...
      followups: active.filter(l => {
        ...
        if (l.currentStep >= 4) return false; // Ya terminó la secuencia
  ...
  const handleConnectGoogle = () => {
    try {
      if (!(window as any).google || !(window as any).google.accounts) {
        alert("El servicio de Google aún está cargando. Espera unos segundos e intenta nuevamente.");
        return;
      }
  ...
  const generateAIEmail = async (lead: Lead) => {
    ...
    try {
      ...
      Hazlo sonar muy humano, sin lenguaje corporativo aburrido, pero profesional. Solo devuelve el cuerpo del correo.`;
  ...
  return (
    <div className={`flex h-screen bg-slate-950 text-slate-100 overflow-hidden font-['Plus_Jakarta_Sans'] transition-all duration-500 ${isSidecarMode ? 'max-w-[420px] mx-auto border-x border-slate-800 shadow-2xl' : ''}`}>
      {!isSidecarMode && (
        <aside className="w-64 border-r border-slate-900 bg-slate-950/80 backdrop-blur-xl p-6 flex flex-col z-20">
          ...
          <div className="space-y-2">
            ...
            <NavButton id="settings" icon={Settings} label="Configuración" />
          </div>
      ...
      <main className="flex-1 flex flex-col relative overflow-hidden bg-slate-950">
        <header className={`flex items-center justify-between px-6 border-b border-slate-900/50 bg-slate-950/80 backdrop-blur-md z-10 ${isSidecarMode ? 'h-16' : 'h-24'}`}>
          <div className="flex items-center space-x-3">
            ...
            <h2 className="font-black text-white text-xl tracking-tight">
              {view === 'dashboard' ? 'Pendientes de Hoy' : view === 'leads' ? 'Base de Leads' : view === 'templates' ? 'Secuencias de Mail' : 'Ajustes'}
            </h2>
          </div>
          <button onClick={() => setIsAddingLead(true)} className="bg-blue-600 text-white p-3 rounded-xl shadow-lg hover:bg-blue-500 transition-all active:scale-95"><Plus size={18} /></button>
        </header>
            ...
            <div className="space-y-10 animate-in fade-in slide-in-from-bottom-2">
              ...
              <div className="space-y-4">
                ...
                {dashboardData.newLeads.length === 0 ? (
                  <div className="bg-slate-900/20 border-2 border-dashed border-slate-900 rounded-[2rem] py-12 text-center">
                    <CheckCircle2 size={32} className="mx-auto text-slate-800 mb-4" />
                    <p className="text-xs font-bold text-slate-700 uppercase">Sin prospectos nuevos</p>
                    <p className="text-[10px] text-slate-500 mt-2">Importa contactos desde Google para comenzar.</p>
                  </div>
                ) : (
                  dashboardData.newLeads.map(lead => (
                    <div key={lead.id} className="bg-slate-900/40 border border-slate-800 p-5 rounded-2xl flex items-center justify-between group hover:border-blue-500/50 transition-all">
                      ...
                      <button onClick={() => openGmailRaw(lead)} className="bg-blue-600 hover:bg-blue-500 text-white px-5 py-2.5 rounded-xl font-black text-[10px] uppercase tracking-widest flex items-center space-x-2 shadow-lg active:scale-95 transition-all">
                        <Send size={14} />
                        <span>Iniciar</span>
                      </button>
                    </div>
              ...
              <div className="space-y-4 pt-4 border-t border-slate-900">
                ...
                {dashboardData.followups.length === 0 ? (
                  ...
                  dashboardData.followups.map(lead => (
                    <div key={lead.id} className="bg-slate-900/40 border border-slate-800 p-6 rounded-2xl group hover:border-amber-500/30 transition-all">
                      <div className="flex items-center justify-between mb-4">
                        <div className="flex items-center space-x-4">
                          ...
                          <div>
                            <h4 className="font-bold text-white text-sm">{lead.name}</h4>
                            <p className="text-[10px] text-slate-500 font-bold uppercase">{lead.company} • Paso {lead.currentStep + 1}</p>
                          </div>
                        </div>
                        <div className="bg-amber-500/10 text-amber-500 text-[8px] font-black px-2 py-1 rounded border border-amber-500/20 uppercase">Toca Seguimiento</div>
                      </div>
                      <div className="flex space-x-2">
                        <button disabled={aiLoading === lead.id} onClick={() => generateAIEmail(lead)} className="flex-1 bg-indigo-600 h-10 rounded-xl flex items-center justify-center space-x-2 text-white font-black text-[10px] uppercase tracking-widest hover:bg-indigo-500 disabled:opacity-50 transition-all">
             ...
             <div className="space-y-4 animate-in fade-in">
                <div className="flex items-center space-x-2">
                   <div className="relative flex-1">
                      ...
                      <input value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="Filtrar base de leads..." className="w-full bg-slate-900 border border-slate-800 rounded-2xl py-4 pl-12 pr-4 text-xs font-bold focus:outline-none focus:ring-1 focus:ring-blue-500" />
                   </div>
                ...
                <div className="grid gap-2">
                  {leads.filter(l => l.name.toLowerCase().includes(searchQuery.toLowerCase()) || l.company.toLowerCase().includes(searchQuery.toLowerCase())).map(lead => (
                    <div key={lead.id} className="p-4 bg-slate-900/40 border border-slate-800 rounded-2xl flex items-center justify-between group hover:bg-slate-900/60 transition-all">
                      <div className="flex items-center space-x-4">
                        <div className={`w-9 h-9 rounded-xl flex items-center justify-center text-xs font-black ${lead.type === 'KDM' ? 'bg-blue-600/10 text-blue-500' : 'bg-amber-600/10 text-amber-500'}`}>{lead.name.charAt(0)}</div>
                        <div>
                          <p className="text-xs font-bold text-white">{lead.name}</p>
                          <p className="text-[9px] text-slate-500 font-bold uppercase">{lead.company} • Paso {lead.currentStep} • {lead.status}</p>
                        </div>
                      </div>
                      <button onClick={() => setLeads(leads.filter(l => l.id !== lead.id))} className="text-slate-700 hover:text-red-500 p-2 opacity-0 group-hover:opacity-100 transition-all"><Trash2 size={14} /></button>
                    </div>
                  ))}
                </div>
             </div>
            ...
            <div className="space-y-10 animate-in fade-in">
               {['KDM', 'Referrer'].map(type => (
                 <div key={type} className="space-y-4">
                    <p className="text-[10px] font-black text-blue-500 uppercase tracking-widest px-1">Secuencia {type}</p>
                    <div className="grid gap-4">
                      {TEMPLATES[type as LeadType].map((temp, i) => (
                        <div key={i} className="bg-slate-900/40 border border-slate-800 p-6 rounded-[2.5rem] space-y-4 shadow-inner">
          ...
          {view === 'settings' && (
            <div className="space-y-8 animate-in fade-in">
              <div className="p-8 bg-slate-900/40 border border-slate-800 rounded-[2.5rem] space-y-6">
                <div className="flex items-center space-x-4">
                  <div className="p-3 bg-blue-600/10 text-blue-500 rounded-2xl"><User size={24} /></div>
                  <h3 className="text-base font-black uppercase tracking-tighter text-white">Perfil</h3>
                </div>
                <div>
                  <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-2 block">Nombre del Remitente</label>
                  <input value={userName} onChange={(e) => setUserName(e.target.value)} placeholder="Tu nombre en la firma" className="w-full bg-slate-800 border border-slate-700 rounded-2xl px-5 py-4 text-sm font-bold focus:outline-none focus:ring-1 focus:ring-blue-500 transition-all" />
                </div>
              </div>

              {/* Depurador de OAuth */}
              <div className="p-8 bg-slate-900/40 border border-slate-800 rounded-[2.5rem] space-y-5">
                <div className="flex items-center space-x-3 text-amber-500">
                  <AlertTriangle size={20} />
                  <h3 className="text-xs font-black uppercase tracking-widest">Ayuda con Google OAuth</h3>
                </div>
                <div className="bg-amber-950/20 border border-amber-500/10 p-5 rounded-3xl space-y-4">
                  <p className="text-[11px] text-amber-100/60 leading-relaxed">Si ves <b>Error 400: invalid_request</b>, debes copiar esta URL exacta y pegarla en los <b>"Orígenes de JavaScript autorizados"</b> de tu proyecto en Google Cloud Console:</p>
                  <div className="flex items-center space-x-2 bg-black/40 p-3 rounded-xl border border-white/5">
                    ...
                    <button onClick={() => {
                        ...
                        alert("URL copiada!");
                      }} className="p-2 text-blue-400 hover:text-white transition-colors"><Info size={16} /></button>
                  </div>
                ...
                <button onClick={handleConnectGoogle} className="w-full py-4 bg-slate-800 hover:bg-slate-700 text-white rounded-2xl font-black text-[10px] uppercase tracking-widest shadow-xl transition-all">
                  {googleToken ? 'Cuenta Vinculada ✓' : 'Vincular Gmail Contacts'}
                </button>
              </div>
            </div>
          )}
        ...
        {isSidecarMode && (
          <footer className="h-24 border-t border-slate-900 bg-slate-950 flex items-center justify-around px-2">
            <button onClick={() => setView('dashboard')} className={`flex flex-col items-center space-y-1 p-3 rounded-2xl transition-all ${view === 'dashboard' ? 'text-blue-500 bg-blue-500/10' : 'text-slate-600'}`}>
              <Calendar size={20} />
              <span className="text-[9px] font-black uppercase">Hoy</span>
            </button>
            <button onClick={() => setView('leads')} className={`flex flex-col items-center space-y-1 p-3 rounded-2xl transition-all ${view === 'leads' ? 'text-blue-500 bg-blue-500/10' : 'text-slate-600'}`}>
              <Users size={20} />
              <span className="text-[9px] font-black uppercase">Base</span>
            </button>
            <button onClick={() => setView('templates')} className={`flex flex-col items-center space-y-1 p-3 rounded-2xl transition-all ${view === 'templates' ? 'text-blue-500 bg-blue-500/10' : 'text-slate-600'}`}>
              <Mail size={20} />
              <span className="text-[9px] font-black uppercase">Mails</span>
            </button>
            <button onClick={() => setView('settings')} className={`flex flex-col items-center space-y-1 p-3 rounded-2xl transition-all ${view === 'settings' ? 'text-blue-500 bg-blue-500/10' : 'text-slate-600'}`}>
              <Settings size={20} />
      ...
      {isSyncingContacts && (
        <div className="fixed inset-0 bg-slate-950/98 backdrop-blur-xl flex items-center justify-center z-50 p-4">
          <div className="bg-slate-900 border border-slate-800 w-full max-w-xl rounded-[2.5rem] shadow-2xl flex flex-col h-[85vh] overflow-hidden animate-in zoom-in duration-200">
            <div className="p-8 border-b border-slate-800 flex items-center justify-between">
              <div>
                <h3 className="text-2xl font-black text-white tracking-tighter">Google Contacts</h3>
                <p className="text-xs text-slate-500 font-bold">{googleToken ? 'Selecciona prospectos para importar' : 'Autorización requerida'}</p>
              </div>
              {!googleToken ? (
                <button onClick={handleConnectGoogle} className="bg-white text-black px-6 py-3 rounded-2xl font-black text-[10px] uppercase tracking-widest active:scale-95 transition-all shadow-xl">Vincular</button>
              ) : (
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-3 custom-scrollbar">
              {!googleToken ? (
                <div className="h-full flex flex-col items-center justify-center text-center px-12 space-y-6">
                  ...
                  <div className="space-y-2">
                    <p className="text-slate-400 font-black uppercase tracking-widest text-[11px]">Sincroniza tu Workspace</p>
                    <p className="text-xs text-slate-600 leading-relaxed font-bold">Importaremos contactos directamente desde tu cuenta para automatizar las secuencias de HakaLab.</p>
                  </div>
                </div>
              ) : isFetchingRealContacts ? (
                <div className="h-full flex items-center justify-center flex-col space-y-4">
                  <Loader2 className="animate-spin text-blue-500" size={40} />
                  <p className="text-[10px] text-slate-600 font-black uppercase tracking-widest">Leyendo libreta de direcciones...</p>
                </div>
              ) : (
                realContacts.filter(c => c.name.toLowerCase().includes(searchQuery.toLowerCase()) || c.email.toLowerCase().includes(searchQuery.toLowerCase())).map(c => (
                  <div key={c.id} onClick={() => {
                    ...
                    <div className="flex items-center space-x-4">
                      <img src={c.photoUrl} className="w-10 h-10 rounded-xl border border-white/5" onError={(e) => (e.currentTarget.src = 'https://ui-avatars.com/api/?name=' + encodeURIComponent(c.name))} />
                      <div>
                    ...
                    <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center transition-all ${selectedContacts.has(c.id) ? 'bg-blue-500 border-blue-500 scale-110 shadow-lg' : 'border-slate-800'}`}>
              ...
              <div className="p-8 border-t border-slate-800 bg-slate-900/50 flex space-x-3">
                <button onClick={() => {
                   const toAdd = realContacts.filter(c => selectedContacts.has(c.id)).map(c => ({
                     id: crypto.randomUUID(), name: c.name, email: c.email, company: c.company || 'Empresa', type: 'KDM' as LeadType, status: 'Active' as LeadStatus, currentStep: 0, lastActionDate: null, createdAt: new Date().toISOString()
                   }));
                   setLeads([...toAdd, ...leads]);
                   setIsSyncingContacts(false);
                   setSelectedContacts(new Set());
                   setView('dashboard');
                }} className="flex-1 py-4 bg-blue-600 text-white rounded-2xl font-black text-[10px] uppercase tracking-widest shadow-xl shadow-blue-900/30 hover:bg-blue-500 transition-all">Importar como KDMs ({selectedContacts.size})</button>
                <button onClick={() => {
                   const toAdd = realContacts.filter(c => selectedContacts.has(c.id)).map(c => ({
                     id: crypto.randomUUID(), name: c.name, email: c.email, company: c.company || 'Empresa', type: 'Referrer' as LeadType, status: 'Active' as LeadStatus, currentStep: 0, lastActionDate: null, createdAt: new Date().toISOString()
                   }));
                   setLeads([...toAdd, ...leads]);
                   setIsSyncingContacts(false);
                   setSelectedContacts(new Set());
                   setView('dashboard');
                }} className="flex-1 py-4 bg-amber-600 text-white rounded-2xl font-black text-[10px] uppercase tracking-widest shadow-xl shadow-amber-900/30 hover:bg-amber-500 transition-all">Importar como Referidores</button>
              </div>
            )}
            
            <button onClick={() => setIsSyncingContacts(false)} className="p-6 text-slate-600 hover:text-white text-[10px] font-black uppercase tracking-widest transition-colors">Cancelar</button>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-slate-950/98 backdrop-blur-xl flex items-center justify-center z-50 p-4">
          <div className="bg-slate-900 border border-slate-800 w-full max-w-sm rounded-[2.5rem] shadow-2xl p-10 space-y-8 animate-in zoom-in duration-200">
            ...
            <form onSubmit={(e) => {
              ...
              <input required name="name" placeholder="Nombre del prospecto" className="w-full bg-slate-800 border border-slate-700 rounded-2xl px-5 py-4 text-sm font-bold focus:outline-none focus:ring-1 focus:ring-blue-500" />
              <input required name="email" type="email" placeholder="Email" className="w-full bg-slate-800 border border-slate-700 rounded-2xl px-5 py-4 text-sm font-bold focus:outline-none focus:ring-1 focus:ring-blue-500" />
              <input required name="company" placeholder="Empresa" className="w-full bg-slate-800 border border-slate-700 rounded-2xl px-5 py-4 text-sm font-bold focus:outline-none focus:ring-1 focus:ring-blue-500" />
              <select name="type" className="w-full bg-slate-800 border border-slate-700 rounded-2xl px-5 py-4 text-sm font-bold">
                <option value="KDM">Key Decision Maker (KDM)</option>
                <option value="Referrer">Referidor</option>
              </select>
              <button type="submit" className="w-full py-5 bg-blue-600 text-white rounded-2xl font-black text-[10px] uppercase tracking-widest mt-4 shadow-xl shadow-blue-900/40 active:scale-95 transition-all">Registrar Lead</button>
            </form>
            <button onClick={() => setIsAddingLead(false)} className="w-full text-slate-600 font-bold uppercase text-[10px] tracking-widest hover:text-white transition-colors">Cerrar</button>
          </div>
        </div>
      )}
    </div>
